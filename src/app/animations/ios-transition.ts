...
function animateBackButton(
  ...
  const FORWARD_TEXT_KEYFRAMES = [
    {
      offset: 0,
      opacity: 0,
      transform: `translate3d(${START_TEXT_TRANSLATE}, ${largeTitleBox.top -
        40}px, 0) scale(2.1)`
    },
    {
      offset: 1,
      opacity: 1,
      transform: `translate3d(${END_TEXT_TRANSLATE}, ${backButtonBox.top -
        46}px, 0) scale(1)`
    }
  ];
  ...
  const FORWARD_ICON_KEYFRAMES = [
    {
      offset: 0,
      opacity: 0,
      transform: `translate3d(${ICON_TRANSLATE}, ${backButtonBox.top -
        41}px, 0) scale(0.6)`
    },
    {
      offset: 1,
  ...
  const enteringBackButtonTextAnimation: Animation = (createAnimation() as any) as Animation;
  const enteringBackButtonIconAnimation: Animation = (createAnimation() as any) as Animation;
...
function animateLargeTitle(
  ...
  const TITLE_START_OFFSET = rtl
    ? `calc(100% - ${largeTitleEl.right}px)`
    : `${largeTitleEl.left}px`;
  ...
  const BACKWARDS_KEYFRAMES = [
    {
      offset: 0,
      opacity: 0,
      transform: `translate3d(${START_TRANSLATE}, ${backButtonBox.top -
        4}px, 0) scale(0.49)`
    },
  ...
  const clonedLargeTitleAnimation = (createAnimation() as any) as Animation;
...
export const iosTransitionAnimation = (
  ...
  try {
    ...
    const { forward, backward } = createLargeTitleTransition(
      (rootAnimation as any) as Animation,
      isRTL,
      backDirection,
      enteringEl,
      leavingEl
    ...
    enteringToolBarEls.forEach(enteringToolBarEl => {
      ...
      const buttons = Array.from(
        enteringToolBarEl.querySelectorAll("ion-buttons,[menuToggle]")
      );
    ...
    if (leavingEl) {
      ...
      if (leavingContentEl) {
        const leavingTransitionEffectEl = shadow(
          leavingContentEl
        ).querySelector(".transition-effect");
    ...
    return (rootAnimation as any) as Animation;
  } catch (err) {
    throw err;
  }
};
