...
export class HotelCityService {
  ...
  private async initPage() {
    if (!this.cityPage) {
      const dCities = await this.hotelService.getHotelCityAsync();
      // const iCities = await this.interhotelService.getci();
      this.cityPage = new CityPage(dCities, []);
    }
  }
  async onSelectCity(isShow, isDomestic = true) {
    if (!this.cityPage) {
      await this.initPage();
    }
    if (!this.cityPage) {
...
function CityPage(domesticCities, interCities, lang = "cn") {
  ...
  function initData(cities) {
    try {
      ...
      cities.sort((c1, c2) => c2.Sequence - c1.Sequence);
      cities = cities
        .filter((it) => it.IsHot)
  ...
  function getPageEle() {
    return document.querySelector(".hotel-city-page-container");
  }
  ...
  function getHtml() {
    ...
    if (internationalCities && internationalCities.length) {
      page.append(segs);
    }
  ...
  function cacheSelectedCities(histories) {
    window.localStorage.setItem(
      "hotel_cached_histories_cities_key",
  ...
  function clearCachedHistories() {
    window.localStorage.removeItem("hotel_cached_histories_cities_key");
  }
    ...
    letters.forEach((l) => {
      ...
      li.onclick = (evt) => {
        ...
        try {
          ...
          if (segs) {
            headerEle = h.clientHeight + segs.clientHeight || 43;
          }
          let y = 0;
