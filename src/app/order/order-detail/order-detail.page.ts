...
export class OrderDetailPage implements OnInit, AfterViewInit {
  ...
  getTravelFlightTrips() {
    ...
    infos.sort((a, b) => new Date(`${a.TakeoffTime}`.replace(/-/g,'/')).getTime() - new Date(`${b.TakeoffTime}`.replace(/-/g,'/')).getTime());
    return infos;
  }
  canSendEmailMsg() {
  ...
  private async getOrderInfo(orderId: string) {
    ...
    if (this.orderDetail) {
      if (
        ...
        tickets.sort((ta, tb) => {
          return (
            new Date(`${tb.InsertTime}`.replace(/-/g,'/')).getTime() -
            new Date(`${ta.InsertTime}`.replace(/-/g,'/')).getTime()
          );
        });
        if (tickets) {
