...
function CityPage(domesticCities, interCities, lang = 'cn') {
  ...
  function onToggleSegmentsPage(isDomestic) {
      ...
      if (isDomestic) {
          ...
          const mainEl = i.querySelector(".main");
          if (mainEl) {
  ...
  function initRightSideMainHtml(recommendCities) {
      ...
      function getHtml() {
          ...
          if (recommendCities) {
              ...
              const enLen = 16;
              const rowLen = 3;
              const rows = getListRows(recommendCities, rowLen, enLen, len, that.lang);
              const list = document.createElement("div");
              list.classList.add("list");
              allInterItems = [];
              rows.forEach((r) => {
  ...
  function cacheSelectedCities(histories) {
      window.localStorage.setItem("flight_cached_histories_cities_key", JSON.stringify(histories));
  }
  function getCachedSelectedCities() {
      try {
          const l = window.localStorage.getItem("flight_cached_histories_cities_key");
          if (l) {
              return JSON.parse(l)
          }
      } catch (e) {
  ...
  function loadMoreItems(infinite = null) {
      ...
      if (kw) {
          let tmpArr;
          if (kw.length == 3) {
              tmpArr = arr.filter(it => (it.Code || "").toLowerCase() == kw.toLowerCase());
          }
          arr = arr.filter(it => {
              if (tmpArr) {
                  return tmpArr.some(one => it.matchStr.toLowerCase().includes(one.CityName.toLowerCase()))
              }
