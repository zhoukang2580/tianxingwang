...
export class FlightCityService {
  constructor(private flightService: FlightService, private interFlightService: InternationalFlightService) { }
  async onSelectCity(isShow = TrainModule) {
    ...
    const cp = new CityPage(dCities, iCities);
    cp.openPage(isShow);
    return new Promise<any>(rsv => {
      cp.onSelectCallback = (obj) => {
        rsv(obj);
      }
    });
  }
}
function CityPage(cities, internationalCities) {
  const tabObj = {
    '推荐': 'isHot',
    '港澳台': 11,
    '东亚': 2,
  ...
  function openPage(open) {
    ...
    if (!page) {
      page = getHtml();
      const fabbtn = document.body.querySelector(".fab-btn.flight-city-page-container");
      page.onscroll = () => {
        if (fabbtn) {
  ...
  function onSelectCity(c) {
    ...
    if (that.onSelectCallback) {
      that.onSelectCallback({ isDomestic: true, city: c });
    }
    console.log(c);
  }
  ...
  function onInterSidebarTabActive(tab) {
    try {
      ...
      let arr = name == 'isHot' ? internationalCities.filter(it => it.IsHot) : internationalCities.filter(it => it.DestinationAreaType == name);
      const h = arr.filter(it => it.IsHot);
      const notH = arr.filter(it => !it.IsHot).sort((a, b) => a.Sequence - b.Sequence);
      arr = h.concat(notH);
  ...
  function getDomesticContainerPage() {
    return getPageEle().querySelector(".domestic.wrapper .domestic-page-container");
  }
  function getInternationalContainerPage() {
    return getPageEle().querySelector(".international.wrapper .international-page-container");
  }
