...
export class SlidvalidateComponent implements OnInit, AfterViewInit {
  ...
  async createImg(onload: () => any) {
    ...
    tempImage.onerror = async () => {
      tempImage.src = await this.getRandomImgSrc(true);
    };
    tempImage.src = await this.getRandomImgSrc();
    return tempImage;
  }
  ...
  async getRandomImgSrc(defaultImage: boolean = false): Promise<string> {
    const defaultImageSrc = "assets/images/train.jpg";
    if (defaultImage) {
      return Promise.resolve(defaultImageSrc);
    }
  ...
  async reset() {
    ...
    this.clean();
    // this.drawGrayLayer();
    this.img.src = await this.getRandomImgSrc();
    this.onRefresh();
  }
}
