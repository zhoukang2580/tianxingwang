...
export class SelectedTripInfoPage implements OnInit, OnDestroy {
  ...
  onReserve() {
    this.router.navigate(["international-flight-book"]);
  }
  onSelectSeat() {
  ...
  getTransferSegments(index, item, transferSegments) {
    if (
      transferSegments[index + 1] &&
      transferSegments[index + 1].FromAirport
    ) {
      // console.log(item.ToAirport, transferSegments[index + 1].FromAirport, "item");
      if (
        item.ToAirport == transferSegments[index + 1].FromAirport &&
        item.ToAirportName == transferSegments[index + 1].FromAirportName
      ) {
        return `${item.ToCityName}  ${item.ToAirport}  ${item.ToAirportName}`;
      }
      return `${item.ToCityName} ${item.ToAirport} ${
        item.ToAirportName
      }   |   ${transferSegments[index + 1].FromCityName} ${
        transferSegments[index + 1].FromAirport
      } ${transferSegments[index + 1].FromAirportName}`;
    }
  }
  getOneWay() {
    if (this.searchModel && this.searchModel.voyageType) {
      if (this.searchModel.voyageType == FlightVoyageType.OneWay) {
        ...
        if (
          this.searchModel.trips[0].fromCity.CountryCode == "CN" &&
          this.searchModel.trips[this.searchModel.trips.length - 1].toCity
            .CountryCode != "CN"
        ) {
          return true;
        }
