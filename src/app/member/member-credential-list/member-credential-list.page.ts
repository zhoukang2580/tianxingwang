...
export class MemberCredentialListPage implements OnInit, OnDestroy {
  ...
  onAdd() {
    this.router.navigate([AppHelper.getRoutePath("member-credential-management")], { queryParams: { addNew: "true" } })
  }
  ngOnInit() {
  ...
  onDetailPage(c: MemberCredential) {
    this.router.navigate([AppHelper.getRoutePath("member-credential-management")], { queryParams: { data: JSON.stringify(c) } })
  }
    ...
    this.credentials = credentials.map(c => {
      return {
        ...
        ExpirationDate: c.ExpirationDate.indexOf("T")
          ? this.calendarService.getMoment(0, c.ExpirationDate).format("YYYY/MM/DD")
          : c.ExpirationDate
      };
    });
